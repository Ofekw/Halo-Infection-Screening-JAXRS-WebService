...
public class CandidateWebServiceTest {
	private static final String WEB_SERVICE_URI = "http://localhost:10000/services/candidates";
...
//		Candidate candidate = new Candidate("A259", "Carter", new Date(17374219200000l), constants.Gender.MALE, null);
//		
//		Response response = client
//				.target(WEB_SERVICE_URI).request()
//				.post(Entity.xml(candidate));
...
//		Candidate fromService = client.target(location).request()
...
//		// creates a Candidate.
//		assertEquals(candidate.getLastname(), fromService.getLastname());
//		assertEquals(candidate.getFirstname(), fromService.getFirstname());
//		assertEquals(candidate.getGender(), fromService.getGender());
//		assertEquals(candidate.getDob(), fromService.getDob());
	}
}
